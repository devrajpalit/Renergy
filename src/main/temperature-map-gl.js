...
(function (global, factory) {
	...
	temperature_map_gl.prototype.set_points = function(points, low_val, high_val, normal_val){
		...
		if(points.length){
			...
			for(var i=0;i < points.length; ++i){
				...
				p[2] = Math.max(Math.min(1., Math.pow((p[2] - avg)/d,1) + 0.5), 0.);
	...
	temperature_map_gl.prototype.draw_points = function(){
		...
		for(var i=0; i < this.points.length; ++i){
			...
			dot.innerHTML = this.point_text(p[3]);
	...
	function celcius_to_rgb(c, temperature_map) {
	...
	temperature_map_gl.prototype.generate_color_map_texture = function(color_map){
		...
		for(var i=0; i < sample_size; ++i){
			...
			var color = hex_to_rgb(celcius_to_rgb(c, color_map));
			ret.push(color.r);
			ret.push(color.g);
			ret.push(color.b);
			ret.push(1);
